...
class GarageEnvFullTest(gym.Env):
    ...
    def __init__(self):
        ...
        #set state
        self.current_demand = demand_stochastic_less(0,self.rD0s, self.rD10s, self.rDfs) #start with static demand only
...
#still non anticipative because action is chosen before observation but better matching of good actions to right period and reward
        ...
        reward = (revenue - opex(self.current_capacity) - self.fixed_cost -E_cost) /(1.12**self.time_steps)
