...
demand_projections = electricity_yearly_demand_stochastic_less_series_ngers(T, n_gers)
cluster_radius_projections = migration_cluster_radius_series_yearly(T)
...
heating_demand_projections = yearly_heat_demand_fromtemp_ngers_stoch_series(area_ger_5_walls , n_gers)
carbon_pricing_projections =  pd.Series(np.zeros((T+1)))
...
class MinigridBasic30yr_stoch_s1_mask_repl_yearly(gym.Env):
    def __init__(self , mismatch_cost): # remember to update all other cost formulas, including in inflexible baseline to reflect this
        ...
        self.eh_repl_yr = []
        self.wind_repl_yr =[]
        self.batt_repl_yr = list(range(0, 30, 4))
    ...
    def step(self, action):
        ...
        # evaluate uncertain parameters
        self.current_demand = electricity_yearly_demand_stochastic_less_ngers(n_gers, self.time_steps,self.rD0s, self.rD10s, self.rDfs) # bring forward timestep, action chosen before this realization but rewards determined by this
        self.current_cluster_radius = migration_cluster_radius(self.time_steps ,self.rD0r, self.rD10r, self.rDfr)
        self.current_heating_demand = heating_demand_projections[self.time_steps]
