...
class MinigridMediumDict(gym.Env):
    ...
    def __init__(self, mismatch_cost):
        ...
        #DEFINE OBSERVATION SPACES HERE TO BE USED WITHIN DICTIONARY
        demand_space = spaces.Box(min_monthly_demand, max_monthly_demand, shape = (1,))
        pv_cap_space = spaces.Discrete(1)
        battery_cap_space = spaces.Discrete(1)
        wind_cap_space = spaces.Discrete(1)
        diesel_cap_space = spaces.Discrete(1)
        inverter_cap_space = spaces.Discrete(1)
        time_left_space = spaces.Discrete(1)
        cluster_radius_space = spaces.Box(min_cluster_radius,max_cluster_radius, shape = (1,))
        cabling_lenght_space = spaces.Box(min_cabling_lenght,max_cabling_lenght, shape = (1,))
        
        
        
        # collate together to yield complete observation space
        self.observation_space = spaces.Dict({"demand" : demand_space, "PV(W)": pv_cap_space , "Battery(kWh)" : battery_cap_space , "Wind(W)": wind_cap_space , "Diesel(W)": diesel_cap_space , "Inverter(W)": inverter_cap_space, "Months to end" : time_left_space , "Cluster radius(m)" : cluster_radius_space , "Available Cable(m)" : cabling_lenght_space})
    ...
    def step(self, action):
        ...
        tranmission_losses = cable_energy_losses_3ph_pct(avg_tranmission_req, self.current_cable_lenght) # this also comes out in kWh
        ...
        mismatch = self.current_demand - total_electricity_delivered
       ...
       # calculate total costs for this year    
        ...
        if self.time_steps == T_monthly:
            ...
            salvage_value_pv = salvage_pv(self.current_pv_capacity, self.time_steps) #the .1 is added to see if it helps agent ovetrbuild less
            salvage_value_wind = salvage_wind(self.current_wind_capacity, self.time_steps)
        ...
        info = {"CF PV":pv_cf , "CF Wind":wind_cf , "Tranmission losses (kWh)" : tranmission_losses , "Distributed Energy(kWh)" : avg_tranmission_req, "Mismatch" : mismatch ,   "Mismatch Pct" : mismatch_pct, "Diesel use (kWh)" :  electricity_generated_diesel , "Diesel CF" : cf_diesel}
