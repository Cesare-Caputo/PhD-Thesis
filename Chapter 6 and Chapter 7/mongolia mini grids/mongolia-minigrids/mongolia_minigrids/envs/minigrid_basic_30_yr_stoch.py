...
from minigrid_cost_rl_oldtruevalues import *
...
carbon_pricing_projections = china_cc_series(Tm + 1)
...
min_eh_capacity = 50 * n_gers # in Watts
...
class MinigridBasic30yr_stoch_s1(gym.Env):
    ...
    def step(self, action):
        ...
        self.current_carbon_price = carbon_pricing_projections[self.time_steps] 
        ...
        diesel_liters_used = monthly_diesel_liters(electricity_generated_diesel)
        ...
        net_grid_cashflow , net_grid_import = grid_electricity_interaction(leftover_heat_demand, leftover_posteh_electricity,  self.time_steps)
        ...
        # this is already included in system opex hence why omitted from below total cost calculations
       ...
       # calculate total costs for this year    
        tot_cost = mismatch_cost + E_cost + system_opex + cabling_exp_cost - carbon_credit_revenue - net_grid_cashflow
        ...
        info = { "Net Grid CF": net_grid_cashflow  , "Carbon Credit Revenue" : carbon_credit_revenue  , "CF PV":pv_cf , "CF Wind": wind_cf , "Electric Heater usage(kWh)": heat_generated_eh,   "Tranmission losses (kWh)" : tranmission_losses , "Distributed Energy(kWh)" : avg_tranmission_req, "Shortage amount (kWh)" : shortage ,   
                "Mismatch Cost" : mismatch_cost, "Diesel use (kWh)" :  electricity_generated_diesel , "Coal Used(kg)" : coal_requirement_for_heat , "System cost" : tot_cost , "Coal cost" : coal_expenditure ,"Expansion Capex" : E_cost , "Opex" : system_opex , 
                "PV Capex" : E_cost_pv , "Wind Capex" : E_cost_wind , "Battery Capex" : E_cost_battery , "Diesel Capex" : E_cost_diesel , "EH Capex" : E_cost_eh , "Inverter Capex" : E_cost_inverter , "Cabling Capex" : E_cost_cabling , "Cabling Lenght": self.current_cable_lenght}
