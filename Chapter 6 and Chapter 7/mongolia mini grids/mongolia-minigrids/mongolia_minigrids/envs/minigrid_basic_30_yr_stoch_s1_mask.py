...
class MinigridBasic30yr_stoch_s1_mask(gym.Env):
    ...
    def step(self, action):
        ...
        if self.time_steps in batt_repl_yr:
            E_cost_battery = battery_capex(self.current_battery_capacity)
            E_cost += E_cost_battery
        elif self.time_steps in pv_repl_yr:
            E_cost_pv = pv_capex(self.current_pv_capacity / 2) 
            E_cost += E_cost_pv
        elif self.time_steps in inverter_repl_yr:
            E_cost_inverter = inverter_capex(self.current_invert_capacity)
            E_cost +=E_cost_inverter
        elif self.time_steps in eh_repl_yr:
            E_cost_eh = eh_capex(self.current_eh_capacity)
            E_cost += E_cost_eh
