...
# d = {'PC0' :PCs[0], 'PC1' :PCs[1] , 'PC2' :PCs[2], 'PC3' :PCs[3] , 'PC4' :PCs[4]}
# df = pd.DataFrame(data=d)
# df.to_csv('sdg7.1.1_complete_v1.csv')
...
axs[0,0].fill_between(years, y['PC0'], y['PC1'], color = 'green', alpha = .4)
axs[0,0].legend()
...
# y2 = eh_cap_df2 / 60000   # pc0
# y3 = eh_cap_df2 / 52000  #pc1
# y4=  eh_cap_df2 / 50000 # pc2
# y5=  eh_cap_df2 / 57000  # pc3
# y6=  eh_cap_df2 / 54000 # pc4
...
#     y2 = y2*np.random.uniform(1.003,1.008)
#     y3 = y3*np.random.uniform(1.004,1.009)
#     y4 = y4*np.random.uniform(1.005,1.012)
#     y5 = y5*np.random.uniform(1.006,1.008)
#     y6 = y6*np.random.uniform(1.005,1.009)
...
#     if y2[i]>.82:
#         y2[i] = np.random.uniform(.82, .84)
#     if y3[i]>.88:
#         y3[i] = np.random.uniform(.83, .89)        
#     if y4[i]>.92:
#         y4[i] = np.random.uniform(.92, .995)
#     if y5[i]>.82:
#         y5[i] = np.random.uniform(.84, .86) 
#     if y6[i]>.86:
#         y6[i] = np.random.uniform(.86, .91) 

# #             pc[4] = np.random.uniform(.86, .91)
...
# d = {'PC0' :PCs[0], 'PC1' :PCs[1] , 'PC2' :PCs[2], 'PC3' :PCs[3] , 'PC4' :PCs[4]}
# df = pd.DataFrame(data=d)
# df.to_csv('sdg7.1.2_complete_v1.csv')
...
axs[0,1].fill_between(years, y['PC0'], y['PC2'], color = 'red', alpha = .4)
axs[0,1].legend()
...
### bat data approach
bat= pd.read_csv('battery_cap_plot_data_ok.csv')
bat.drop(columns = ['Unnamed: 0'], axis = 1, inplace = True)
#plt.plot(bat)
#bat = bat['0'].values.flatten()



eh_cap_df2 = bat['0'].values.flatten()
y2 = eh_cap_df2 / 600  # pc0
y3 = eh_cap_df2 / 560  #pc1
y4=  eh_cap_df2 / 530 # pc2
y5=  eh_cap_df2 / 610  # pc3
y6=  eh_cap_df2 / 585 # pc4
...
eh_cap_df2 = wind['0'].values.flatten()
#eh_cap_df2 = pv['0'].values.flatten()


y2 = eh_cap_df2 / 9000  # pc0
y3 = eh_cap_df2 / 8900  #pc1
y4=  eh_cap_df2 / 8960 # pc2
y5=  eh_cap_df2 / 8990  # pc3
y6=  eh_cap_df2 / 8910 # pc4
...
##### note do this but with values smaller than 1 for all ########

for i in range(50,150):
    y2 = y2*np.random.uniform(.991,.995)
    y3 = y3*np.random.uniform(.995,.998)
    y4 = y4*np.random.uniform(.990,.993)
    y5 = y5*np.random.uniform(.991,.993)
    y6 = y6*np.random.uniform(.994,.997)


# for i in range(80,350):
#     y2[i] = y2[i-1]*np.random.uniform(1.003,1.008)
#     y3[i] = y3[i-1]*np.random.uniform(1.005,1.012)
#     y4[i] = y4[i-1]*np.random.uniform(1.004,1.009)
#     y5[i] = y5[i-1]*np.random.uniform(1.006,1.008)
#     y6[i] = y6[i-1]*np.random.uniform(1.005,1.009)

for i in range(80,360):
    y2[i] = y2[i-1]*np.random.uniform(1.03,1.08)
    y3[i] = y3[i-1]*np.random.uniform(1.05,1.12)
    y4[i] = y4[i-1]*np.random.uniform(1.04,1.09)
    y5[i] = y5[i-1]*np.random.uniform(1.06,1.08)
    y6[i] = y6[i-1]*np.random.uniform(1.05,1.09)
...
#     for pc in PCs:
#         if pc[i]<.2:
#             pc[i] = np.random.uniform(.95, .98)

for i in range(80,360):
    for pc in PCs:
        if pc[i]>1.0:
            pc[i] = np.random.uniform(.88, .98)
...
for i in range(80,360):
        y2[i] = np.random.uniform(.88, .91)
        y3[i] = np.random.uniform(.94, .99)        
        y4[i] = np.random.uniform(.89, .91)
        y5[i] = np.random.uniform(.93, .97) 
        y6[i] = np.random.uniform(.92, .96) 
...
# d = {'PC0' :PCs[0], 'PC1' :PCs[1] , 'PC2' :PCs[2], 'PC3' :PCs[3] , 'PC4' :PCs[4]}
# df = pd.DataFrame(data=d)
# df.to_csv('sdg7.2_complete_v1.csv') 
...
axs[1,0].plot(years, y['PC2'],  color = 'red' , label = 'PC2', ls='--')
...
##### SDG 7.b.1 #### RE per capita
#bat2= bat*4.5
#wt = wt/18
wind = pd.read_csv('wind_cap_plot_data.csv')
wt = wind['1']
wt = pd.Series(wt)
for i in range(25,360):
    if wt.iloc[i] < wt.iloc[i-1]:
        r= np.random.uniform(1.0001, 1.0005)
        wt.iloc[i] = wt.iloc[i-1] *r
#plt.plot(wt)

wt = gaussian_filter1d(wt, sigma=2)
plt.plot(wt)

wt = wt/18
caps = []
caps=[wt, wt*1.13, wt*1.03, wt*1.09, wt*1.19]
for i in range(100,250):
    for cap in caps:
...
d = {'PC0' :PCs[0], 'PC1' :PCs[1] , 'PC2' :PCs[2], 'PC3' :PCs[3] , 'PC4' :PCs[4]}
df = pd.DataFrame(data=d)
df.to_csv('sdg7.b.1_complete_v1.csv') 
