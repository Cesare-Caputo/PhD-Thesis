...
#THIS IS BASELINE COST
# SEPARATE FUNCTIONS BELOW IMPLEMENTED ONLY ON VARIABLES WHERE POLICY CHANGE HAS AN IMPACT
...
def pv_capex_s2(capacity):
    #this calculated cost only, setting up inflexible optimisation
    capex = pv_cost*capacity *.5
    return capex

def wind_capex_s2(capacity):
    #this calculated cost only, setting up inflexible optimisation
    capex = wind_cost*capacity *.5
    return capex

# PC3
def eh_capex_s3(capacity):
    cost = eh_capex_w*capacity *.5
    return cost

# PC6

def pv_capex_s6(capacity):
    #this calculated cost only, setting up inflexible optimisation
    capex = pv_cost*capacity *.5
    return capex

def wind_capex_s6(capacity):
    #this calculated cost only, setting up inflexible optimisation
    capex = wind_cost*capacity *.5
    return capex

# PC3
def eh_capex_s6(capacity):
    cost = eh_capex_w*capacity *.5
    return cost
