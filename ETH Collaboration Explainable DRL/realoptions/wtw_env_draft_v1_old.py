...
class WtwEnvSimple_v1_old(gym.Env):
    ...
    def __init__(self):
        ...
        supply, demand = self.wtw.balance_rl(self.producers, self.consumers, [self.base], 0) #first timestep
    ...
    def step(self, action):
      ...
      #NOTE THAT DOING SO MEANS WE HAVE TO KEEP RECORD OF OPTIONS TAKEN... 
        if action == 0:
            options = [self.base]
            supply, demand =  self.wtw.balance_rl(self.producers, self.consumers, options , self.time_steps)
            self.total_demand = np.sum(demand.get("data"), keepdims=True)
            self.total_supply = np.sum(supply.get("data"), keepdims=True)
        elif action == 1:
            raw_20 = OptionSupply(
