...
class WaterSupplyRL(BaseModel):
    ...
    def balance_rl(self, producers, consumers, options , timestep):
        """Balance supply and demand"""
        # pylint: disable=too-many-locals
        ...
        # total demand and supply
        total_demand = np.sum(demand, axis=1, keepdims=True)#.flatten()[timestep]
        total_supply = np.sum(supply, axis=1, keepdims=True)#.flatten()[timestep]
        ...
        return ({'data': supply[timestep], 'labels': s_labels},
                {'data': demand[timestep], 'labels': d_labels})
